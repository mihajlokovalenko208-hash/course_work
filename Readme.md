...
4.Реалізувати архівування телеметричних даних у Google Sheets.
5.Створити веб-інтерфейс для моніторингу параметрів системи.
6.Реалізувати систему автоматичних сповіщень через Discord.
7.Забезпечити автоматичне керування поливом на основі показників вологості ґрунту.
...
2. Датчик вологості ґрунту: Capacitive Soil Moisture Sensor v1.2
   * *Обґрунтування*: На відміну від дешевих резистивних датчиків, цей датчик є ємнісним. Його контакти покриті лаком і не контактують із водою напряму, що повністю виключає корозію та забезпечує довговічність роботи в землі.

3. Аналогово-цифровий перетворювач (АЦП): MCP3008
   * *Обґрунтування*: Оскільки Raspberry Pi не має власних вбудованих аналогових входів , для зчитування точних значень напруги з ємнісного датчика ґрунту обрано 10-бітний 8-канальний АЦП MCP3008, який працює по надійному протоколу SPI.
...
Для контролю роботи системи використовується накопичення, візуалізація та архівування даних у середовищі Node-RED. Значення вологості ґрунту та температури передаються на інформаційну панель Dashboard, де відображаються у вигляді графіків та індикаторів.

Для довготривалого зберігання інформації використовується інтеграція з сервісом Google Sheets. Після кожного циклу опитування датчиків система автоматично формує запис, який містить дату, час, значення вологості ґрунту та температури повітря. Дані передаються через вузол Google Sheets у Node-RED та зберігаються у хмарній таблиці.

Такий підхід дозволяє вести журнал роботи системи, аналізувати статистику за тривалий період часу та використовувати накопичені дані для подальшої оптимізації алгоритмів автоматичного поливу.

### 2.5. Технічна структура системи
Проєкт системи контролю поливу в саду побудований на базі мікрокомп'ютера Raspberry Pi, який виступає в ролі Edge-вузла.
...
* АЦП MCP3008 оцифровує отримані дані та через інтерфейс SPI передає їх  мікрокомп'ютеру Raspberry Pi для подальшого аналізу.
...
Для реалізації програмної логіки системи використовується середовище Node-RED. У ньому реалізовано генерацію тестових даних, обробку телеметрії, передачу повідомлень через MQTT, архівування даних у Google Sheets, роботу алгоритму автоматичного поливу та відображення інформації на Dashboard.

Як організовано збереження даних: Після кожного циклу вимірювання Node-RED формує запис із датою, часом, значеннями вологості ґрунту та температури повітря. Отримані дані автоматично передаються до Google Sheets, де накопичуються для подальшого аналізу та побудови статистики.
...
Використані джерела: 
 ...
 6.Google Sheets API Documentation — документація щодо інтеграції та роботи з сервісом Google Sheets.
