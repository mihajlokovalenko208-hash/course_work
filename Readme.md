...
Програма складається з кількох потоків. Потік IO взаємодіє з обладнанням: зчитує дані з датчика вологості ґрунту через MCP3008 і датчика DHT22, а також керує реле електромагнітного клапана.
...
Потік Telemetry обробляє отримані дані, перетворюючи показники АЦП у відсотки вологості, виконує нормалізацію температурних значень та формує єдиний набір поточних параметрів системи для подальшого використання іншими модулями.

Потік UI відповідає за відображення даних та реалізацію локальної панелі керування системою поливу. Він забезпечує побудову графіків у реальному часі, відображення поточних значень датчиків та можливість ручного керування виконавчим пристроєм.

Потік Report відповідає за накопичення та передачу даних у хмарне середовище. Він формує структуровані пакети інформації та надсилає їх до Google Sheets через API для подальшого аналізу та збереження історії роботи системи.

Потік Bot забезпечує взаємодію з користувачем через Discord-бота. Він надсилає сповіщення про критичні значення вологості та стан системи, а також приймає команди дистанційного керування поливом (наприклад, увімкнення або вимкнення клапана).


<img width="964" height="534" alt="image" src="https://github.com/user-attachments/assets/76efd16b-ab58-4a93-bbf9-1cf551efc2d5" />

### 


<img width="1000" height="108" alt="image" src="https://github.com/user-attachments/assets/8954c6c1-7247-48fb-af71-cde855f1ab78" />




Для обміну даними між потоками використовуються контексти Node-RED. Контекст RTDB зберігає поточні показники датчиків та стани системи, а контекст RPRT використовується для накопичення даних перед передачею у хмарне середовище.
...
На нижньому рівні дані від датчика вологості ґрунту та DHT22 через потік IO надходять у Telemetry, де відбувається їх обробка та нормалізація. Після цього сформовані дані використовуються іншими модулями системи.

У разі досягнення критичних значень вологості потік Telemetry ініціює передачу повідомлень до Discord-бота через потік Bot, а також оновлення стану на локальному інтерфейсі UI.

Паралельно потік Report фіксує всі зміни параметрів системи та періодично передає їх у хмарне сховище Google Sheets для подальшого аналізу.

### 4.3. ПЗ для хмарних рішень

Оскільки Edge-рівень має обмежені ресурси пам’яті, частина обробки та довготривалого зберігання даних винесена у хмарне середовище.

Google Sheets

Хмарне збереження реалізовано через інтеграцію з Google Sheets за допомогою API. Потік Report передає сформовані набори даних, які автоматично записуються в онлайн-таблицю.

Це дозволяє користувачу переглядати історію показників вологості ґрунту, температури та стану поливу з будь-якого пристрою, підключеного до Інтернету.

Discord-бот

Для оперативного інформування та дистанційного керування системою реалізовано Discord-бота. Через потік Bot система надсилає повідомлення про критичний стан вологості ґрунту або початок автоматичного поливу.

Також реалізовано можливість віддаленого керування системою через текстові команди, наприклад увімкнення або вимкнення поливу.

### 4.4. WEB-інтерфейси (локальний для Edge та глобальний)

Графічне відображення процесів керування системою поділено на два рівні залежно від способу доступу користувача.

### 4.4.1. Локальний WEB-інтерфейс (Edge Dashboard)

Локальний інтерфейс реалізований засобами Node-RED Dashboard та доступний у межах локальної мережі. За його роботу відповідає потік UI.

На головному екрані відображаються поточні значення вологості ґрунту та температури повітря, графіки зміни параметрів у реальному часі, а також кнопка ручного керування реле з індикатором стану клапана.

Додатково передбачено режим імітації, який дозволяє перевіряти роботу системи без підключення фізичних датчиків за допомогою повзунків (sliders).

### 4.4.2. Глобальний WEB-інтерфейс (Global Dashboard)

Глобальний інтерфейс реалізований на базі хмарних сервісів і забезпечує віддалений доступ до даних системи через Інтернет.

Його основне призначення — відображення довгострокової аналітики роботи системи, включаючи зміни вологості ґрунту, температурних показників та статистику поливу.

Це дозволяє користувачу контролювати стан системи з будь-якого пристрою без необхідності підключення до локальної мережі.
